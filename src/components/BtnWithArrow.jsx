...
const BtnWithArrow = ({ children, direction = "right", toggleable = false, onClick, size = "normal" }) => {
  ...
  return (
    <button onClick={handleClick} className={`text-sm sm:text-md md:text-lg group inline-flex gap-2 text-primary-red transform`}>
      {isLeft && (
        <div className={`relative translate-y-[0.5em] ${arrowHeight} ${arrowWidth} transition-all duration-300 ${hoverWidth}`}>
          <span className={`absolute top-1/2 right-0 h-0.5 w-full bg-primary-red transform -translate-y-1/2`} />
          <span className={`absolute top-1/2 left-0 ${arrowHead} border-t-2 border-l-2 border-primary-red transform -rotate-45 translate-y-[-50%]`} />
        </div>
      )}
      ...
      {!isLeft && (
        <div className={`relative translate-y-[0.5em] ${arrowHeight} ${arrowWidth} transition-all duration-300 ${hoverWidth}`}>
          <span className={`absolute top-1/2 left-0 h-0.5 w-full bg-primary-red transform -translate-y-1/2`} />
          <span className={`absolute top-1/2 right-0 ${arrowHead} border-t-2 border-r-2 border-primary-red transform rotate-45 translate-y-[-50%]`} />
        </div>
      )}
    </button>
  );
